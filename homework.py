...
class Training:
    ...
    def get_spent_calories(self) -> float:
        ...
        raise NotImplementedError(
            f'Определите run в {type(self).__name__.__name__}.')
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        return (self.MIN_IN_H * self.duration
                * (self.CALORIES_WEIGHT_MULTIPLIER_1 * self.weight
                   + (self.KM_H_IN_M_S * self.get_mean_speed())**2
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        return ((self.CALORIES_MEAN_SPEED_SHIFT + self.get_mean_speed())
